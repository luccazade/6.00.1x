...
class MITCampus(Campus):
    ...
    def get_tents(self):
        ...
        rawTents = []
        for tent in self.tentList:
            rawTents.append((tent.getX(), tent.getY()))
        sortTents = sorted(rawTents, key=lambda x: x[0])
        formTents = []
        for tent in sortTents:
            tent = Location(tent[0], tent[1])
            formTents.append(tent.__str__())
        return formTents
...
print(c.add_tent(Location(2, 3)))
# print(c.remove_tent(Location(2,3)))
